...
הוחלט ומומש (עדכון 30.08.2026):

* שדות טופס ההזמנה: מוצרים (בחירה מובנית עם כמות לכל מוצר), שם מלא, טלפון, מייל, כתובת למשלוח, הערות למשלוח (לא חובה).
* מערכת ההזמנות מלאה ב-n8n ("Ordering System", תיקיית Bread And Sun): Webhook → תיוק ב-Google Sheets עם מספר הזמנה רץ → מייל לבית העסק → מייל תודה ללקוח → תשובה לאתר. ה-webhook URL מוגדר ב-`js/script.js` (`CONFIG.orderEndpoint`) ואינו placeholder.
* קובץ הגוגל שיטס "לחם ושמש – הזמנות" (גיליון "הזמנות") עם עמודות: מספר הזמנה, תאריך ושעה, שם מלא, טלפון, מייל, כתובת, מוצרים, הערות למשלוח. מעוצב RTL, שורת כותרת קפואה ומודגשת.
